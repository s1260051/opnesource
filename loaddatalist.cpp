...
LoadDataList::LoadDataList(QWidget *parent)
    : QDialog(parent), ui(new Ui::LoadDataList) {
  ui->setupUi(this);
  this->setWindowTitle("Load Data List");
  this->move(10, 300);

  QObject::connect(ui->clearListButton, SIGNAL(clicked()), ui->loadedDataList,
                   SLOT(clear()));
}

LoadDataList::~LoadDataList() { delete ui; }

void LoadDataList::on_loadedDataList_clicked(const QModelIndex &index) {

  emit SelectDataSignal(fileInfo[index.row()],
                        ui->loadedDataList->item(index.row())->text().length());
}

void LoadDataList::on_tilingButton_clicked() { emit tilingWindowSignal(); }

void LoadDataList::on_closeAllButton_clicked() { emit closeWindowSignal(); }

void LoadDataList::on_loadFileButton_clicked() {

  QString fileName =
      QFileDialog::getOpenFileName(this, tr("Open Image"), QDir::homePath(),
                                   tr("Image Files (*.png *.jpg *.bmp *.img "
                                      "*.fit *.fits *.fts *.txt *.dat *.inf)"));
  ...
  for (int i = 1; i < fileName.length(); i++) {
    n = i - 1;
    if (!QString::compare(fileName.mid(fileName.length() - i, 1), d))
      break;
  }
...
void LoadDataList::on_changeParameterButton_clicked() {
  ...
  emit changeParameterSignal(ui->minT->text().toDouble(),
                             ui->maxT->text().toDouble(),
                             ui->changeColor->currentIndex());
}

void LoadDataList::on_substract_clicked() { emit substractSignal(); }
