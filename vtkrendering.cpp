...
VtkRendering::VtkRendering(QWidget *parent) : QOpenGLWidget(parent) {
...
void VtkRendering::loadFile() {
  ...
  if (!filep.open(QIODevice::ReadOnly) || !filef.open(QIODevice::ReadOnly)) {
    printf("error\n");
    return;
  }
...
void VtkRendering::loadVtkFile(QString vtkFileName) {
  ...
  for (int i = 0; !vtk.atEnd(); i++) {
    ...
    if (73741 <= i && i < 73740 + NUM) {
      vtk >> str;
      if (str.toDouble() > MAX[0])
        MAX[0] = str.toDouble();
      if (str.toDouble() < MIN[0])
        MIN[0] = str.toDouble();
      ...
      vtk >> str;
      if (str.toDouble() > MAX[1])
        MAX[1] = str.toDouble();
      if (str.toDouble() < MIN[1])
        MIN[1] = str.toDouble();
      ...
      vtk >> str;
      if (str.toDouble() > MAX[2])
        MAX[2] = str.toDouble();
      if (str.toDouble() < MIN[2])
        MIN[2] = str.toDouble();
      ...
      vtk >> str;
      if (str.toDouble() > MAX[3])
        MAX[3] = str.toDouble();
      if (str.toDouble() < MIN[3])
        MIN[3] = str.toDouble();
...
void VtkRendering::getDirPath(QString n) { dirPath = n; }
