...
MainWindow::MainWindow(QWidget *parent)
  ...
  QObject::connect(ui->imageViewer, SIGNAL(valueChangedPixel(QString)), this,
                   SLOT(makePixelInfoList(QString)));

  QObject::connect(this, SIGNAL(loadFileSignal(QString)), ui->imageViewer,
                   SLOT(loadFileName(QString)));

  QObject::connect(ui->exportWindowButton, SIGNAL(clicked()), this,
                   SLOT(exportWindow()));
...
void MainWindow::on_action_Open_triggered() {
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Open Image"), ".", tr("Image Files (*.png *.jpg *.bmp *.img)"));
...
void MainWindow::on_loadDataButton_clicked() {
  ...
  exportFileName =
      ui->loadedDataList->item(ui->loadedDataList->count() - 1)->text();
}
...
void MainWindow::loadFile(QString name) { emit loadFileSignal(name); }

void MainWindow::exportWindow() {
  bool judge = true;
  for (int i = 0; i < ui->windowList->count(); i++) {
    if (ui->loadedDataList->currentItem()->text() ==
        ui->windowList->item(i)->text()) {
      judge = false;
      break;
    }
  }
...
void MainWindow::on_tilingWindowButton_clicked() { emit tilingWindowSignal(); }
