...
int main(int argc, char *argv[]) {
  ...
  QObject::connect(&w, SIGNAL(exportDataSignal(QString)), &tt,
                   SLOT(tilingWindow(QString)));
  ...
  QObject::connect(&ls, SIGNAL(loadDataSignal(QString, int)), &p,
                   SLOT(getFileName(QString, int)));
  QObject::connect(&ls, SIGNAL(SelectDataSignal(QString, int)), &p,
                   SLOT(getFileName(QString, int)));
  ...
  QObject::connect(&ls, SIGNAL(changeParameterSignal(double, double, int)), &p,
                   SLOT(changeParameter(double, double, int)));
  QObject::connect(&ls, SIGNAL(substractSignal()), &p, SLOT(substract()));

  QObject::connect(&d, SIGNAL(infoSignal(QString *)), &i,
                   SLOT(getInfo(QString *)));
  QObject::connect(&d, SIGNAL(getFilePathSignal(QString)), &cali,
                   SLOT(getDataPath(QString)));
  QObject::connect(&d, SIGNAL(getFilePathSignal(QString)), &cg,
                   SLOT(getDataPath(QString)));
  QObject::connect(&d, SIGNAL(getFilePathSignal(QString)), &cgp,
                   SLOT(getDataPath(QString)));

  QObject::connect(
      &cali,
      SIGNAL(showCalibrationGraphSignal(QVector<QVector<QString>>, QString,
                                        QString, QString, int,bool)),
      &cg,
      SLOT(popCalibrationGraph(QVector<QVector<QString>>, QString, QString,
                               QString, int,bool)));
